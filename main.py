...
class KookBotX:
    ...
    def configure_logger(self, log_path: Optional[Union[str, Path]] = None):
        ...
        log_path = (
            Path(log_path) if log_path is not None else (Path(__file__).parent / "logs")
        )
        ...
        logger.add(
            ...
            retention="4 weeks",
            rotation="1 month",
            level="DEBUG" if os.environ.get("KOOKBOTX_DEBUG") == "1" else "INFO",
        )
    ...
    def load_modules(self, module_path: Optional[Union[str, Path]] = None):
        ...
        module_path = (
            ...
            else (Path(__file__).parent / "modules")
        )
        ...
        for module_p in module_path.glob("*"):
            if module_p.is_file() and module_p.suffix == ".py":
                ...
                if module_p.is_dir():
                    logger.warning(
                        "Skipping directory {}: no __init__.py found under this directory",
                        module_p,
                    )
                else:
            ...
            try:
                ...
                logger.warning(
                    "Failed to import module {} ({}): {}", module_name, module_p, e
                )
                continue
            try:
                _ = module.init(bot)
...
if __name__ == "__main__":
    ...
    if not os.environ.get("KOOKBOT_WS_TOKEN"):
        logger.error(
            "KOOKBOT_WS_TOKEN is required but not found as an environment variable, exiting..."
        )
        logger.debug(
            "To set the token, either run `export KOOKBOT_WS_TOKEN=your_token`, or invoke this script with `KOOKBOT_WS_TOKEN=your_token python3 {}`",
            __file__,
        )
        sys.exit(1)
